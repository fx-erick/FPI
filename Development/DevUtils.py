...
def load_model_from_json_path(model_json_path, load_state_dict=True):
    """
...
class AccuracySegmentation(torch.nn.Module):
    ...
    def forward(self, outputs, targets):
        if outputs.shape[0] != targets.shape[0]:
            raise ValueError("Error in Accuracy Seg.: output batch dim must equal target batch dim")
...
# ========================================================================================
# shrutika's method for filter pruning
# ========================================================================================
            ...
            for i in range(0, clustered[kkey]["n_items"]):
                ...
                for n in range(0, clustered[kkey]["n_items"]):
                    if i != n:
                        ...
                        # TODO: apart from the crazy amount of time this needs, this implementation of correlation
                        #  coefficient is actually not correct ... see also:
